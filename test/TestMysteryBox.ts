  ...
  const transferAmount = ethers.utils.parseUnits('100000000', 18).toString();
    ...
    before(async () => {
...

        {
            ...
            expect(result).to.have.property('creator').that.to.be.eq(await user_1.getAddress());
    ...
    it('Should openBox & getBoxInfo work', async () => {
        ...
        const userNftBalance = await enumerableNftContract.balanceOf(await user_2.getAddress());
        {
            ...
            expect(boxInfo).to.have.property('creator').that.to.be.eq(await user_1.getAddress());
        ...
        if (true) {
            ...
            expect(newUserNftBalance.eq(userNftBalance.add(1))).to.be.true;
            {
                ...
                const result = parsedLog.args;
                expect(result).to.have.property('from').that.to.be.eq(await user_1.getAddress());
                expect(result).to.have.property('to').that.to.be.eq(await user_2.getAddress());
            ...
            }
            {
                ...
                expect(result).to.have.property('customer').that.to.be.eq(await user_2.getAddress());
                expect(result).to.have.property('nft_address').that.to.be.eq(enumerableNftContract.address);
    ...
    it('Should sell "not_sell_all" enumerableNftContract work', async () => {
        ...
        {
            await testTokenAContract.transfer(await signers[5].getAddress(), transferAmount);
            await testTokenAContract.connect(signers[5]).approve(mbContract.address, transferAmount);
            await testTokenAContract.transfer(await signers[6].getAddress(), transferAmount);
            await testTokenAContract.connect(signers[6]).approve(mbContract.address, transferAmount);
            await testTokenAContract.transfer(await signers[7].getAddress(), transferAmount);
            await testTokenAContract.connect(signers[7]).approve(mbContract.address, transferAmount);
            await testTokenAContract.transfer(await signers[8].getAddress(), transferAmount);
    ...
    it('Should work if "NFT tokens transfered elsewhere"', async () => {
        ...
        await testTokenAContract.transfer(await testAccount.getAddress(), transferAmount);
    ...
    it('Should claimPayment work', async () => {
        ...
        const authorTokenABalanceBeforeClaim = await testTokenAContract.balanceOf(await user_1.getAddress());
        ...
        const authorTokenABalanceAfterClaim = await testTokenAContract.balanceOf(await user_1.getAddress());
        ...
        await mbContract.connect(user_1).claimPayment([not_sell_all_box_id]);
        {
            ...
            // token A balance after
            const authorTokenABalanceAfterClaimAgain = await testTokenAContract.balanceOf(await user_1.getAddress());
    ...
    it('Should addAdmin/addWhitelist work', async () => {
        ...
        await expect(mbContract.connect(user_1).addWhitelist([await user_test.getAddress()])).to.be.rejectedWith('not admin');
